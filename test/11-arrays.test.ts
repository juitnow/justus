...
describe('Array validator', () => {
  it('should validate an array', () => {
    ...
    expect(() => validate(array, 123)).toThrow((assert) =>
      assert
        .toBeError(ValidationError, /^Found 1 validation error/)
        .toHaveProperty('errors', expect.toMatchContents([{ path: [], message: 'Value is not an "array"' }])),
    )
  })
  ...
  it('should validate an array of a specified length', () => {
    ...
    expect(() => validate(length, [1, 2])).toThrow((assert) =>
      assert
        .toBeError(ValidationError, /^Found 1 validation error/)
        .toHaveProperty(
          'errors',
          expect.toMatchContents([{ path: [], message: 'Array must have a minimum length of 3' }]),
        ),
    )
    expect(() => validate(length, [1, 2, 3, 4, 5, 6, 7])).toThrow((assert) =>
      assert
        .toBeError(ValidationError, /^Found 1 validation error/)
        .toHaveProperty(
          'errors',
          expect.toMatchContents([{ path: [], message: 'Array must have a maximum length of 6' }]),
        ),
    )

    expect(validate(array({ minItems: 3, maxItems: 3 }), [1, 2, 3])).toEqual([1, 2, 3])

    expect(() => array({ minItems: -1 })).toThrowError(TypeError, 'Constraint "minItems" (-1) must be non-negative')
    expect(() => array({ maxItems: -1 })).toThrowError(TypeError, 'Constraint "maxItems" (-1) must be non-negative')
    expect(() => array({ minItems: 4, maxItems: 3 })).toThrowError(
      TypeError,
      'Constraint "minItems" (4) is greater than "maxItems" (3)',
    )
  })
  ...
  it('should validate an array with unique items', () => {
    ...
    expect(() => validate(array({ uniqueItems: true }), [1, 2, 1, 2])).toThrow((assert) =>
  ...
  it('should validate an array with items of a specific type', () => {
    ...
    expect(() => validate(arrayOf(string), ['a', true, 'b', 123])).toThrow((assert) =>
      assert.toBeError(ValidationError, /^Found 2 validation errors/).toHaveProperty(
        'errors',
        expect.toMatchContents([
    ...
    expect(() => validate(arrayOf('a'), ['a', true, 'b', 123])).toThrow((assert) =>
    ...
    expect(() => validate(array({ items: 'a', uniqueItems: true }), ['a', true, 'a', 123])).toThrow((assert) =>
      assert.toBeError(ValidationError, /^Found 3 validation errors/).toHaveProperty(
        'errors',
        expect.toMatchContents([
          { path: [1], message: 'Value does not match constant "a" (string)' },
