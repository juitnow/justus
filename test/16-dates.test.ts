...
describe('Date validator', () => {
  ...
  it('should validate an ISO date', () => {
    ...
    expect(() => validate(validator, 0)).toThrow((assert) =>
      assert.toBeError(ValidationError, /^Found 1 validation error/).toHaveProperty(
        'errors',
        expect.toMatchContents([
          {
            path: [],
            message: 'ISO Date is not a "string"',
          },
        ]),
      ),
    )

    expect(() => validate(validator, 'Thu, 01 Jan 1970 00:00:00 GMT')).toThrow((assert) =>
      assert.toBeError(ValidationError, /^Found 1 validation error/).toHaveProperty(
        'errors',
        expect.toMatchContents([
          {
            path: [],
            message: 'Invalid format for ISO Date',
          },
        ]),
      ),
    )
    ...
    expect(() => validate(validator, 'Thu, 01 Jan 1970 00:00:00 GMT')).toThrow((assert) =>
      assert.toBeError(ValidationError, /^Found 1 validation error/).toHaveProperty(
        ...
        expect.toMatchContents([
          {
            path: [],
            message: 'Unix Timestamp is not a "number"',
          },
        ]),
      ),
    )
  ...
  it('should validate a date within a range', () => {
    ...
    expect(() => validate(v1, 1999)).toThrow((assert) =>
      assert.toBeError(ValidationError, /^Found 1 validation error/).toHaveProperty(
        'errors',
        expect.toMatchContents([
          {
            path: [],
            message: 'Date is before 1970-01-01T00:00:02.000Z',
          },
        ]),
      ),
    )

    expect(() => validate(v1, 3001)).toThrow((assert) =>
      assert.toBeError(ValidationError, /^Found 1 validation error/).toHaveProperty(
        'errors',
        expect.toMatchContents([
          {
            path: [],
            message: 'Date is after 1970-01-01T00:00:03.000Z',
          },
        ]),
      ),
    )
    ...
    expect(() => validate(v2, 1999)).toThrow((assert) =>
      assert.toBeError(ValidationError, /^Found 1 validation error/).toHaveProperty(
        'errors',
        expect.toMatchContents([
          {
    ...
    expect(() => validate(v2, 2001)).toThrow((assert) =>
      assert.toBeError(ValidationError, /^Found 1 validation error/).toHaveProperty(
        'errors',
        expect.toMatchContents([
          {
            path: [],
            message: 'Date is after 1970-01-01T00:00:02.000Z',
          },
        ]),
      ),
    )

    expect(() => date({ from: new Date(100), until: new Date(99) })).toThrowError(
      TypeError,
      'Constraint "until" (1970-01-01T00:00:00.099Z) must not be before "from" (1970-01-01T00:00:00.100Z)',
    )
  })

  it('should consider edge cases', () => {
    expect(() => validate(date, Symbol())).toThrow((assert) =>
      assert.toBeError(ValidationError, /^Found 1 validation error/).toHaveProperty(
        'errors',
        expect.toMatchContents([
          {
            path: [],
            message: 'Value could not be converted to a "Date"',
          },
        ]),
      ),
    )

    expect(() => validate(date, Number.POSITIVE_INFINITY)).toThrow((assert) =>
      assert.toBeError(ValidationError, /^Found 1 validation error/).toHaveProperty(
        'errors',
        expect.toMatchContents([
          {
