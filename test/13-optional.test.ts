...
describe('Object modifiers', () => {
  it('should validate a simple optional validation', () => {
    ...
    expect(() => validate(validation, 'wrong')).toThrow((assert) =>
      assert
        .toBeError(ValidationError, /^Found 1 validation error/)
        .toHaveProperty(
          'errors',
          expect.toMatchContents([{ path: [], message: 'Value does not match constant "foobar" (string)' }]),
        ),
    )
  })
  ...
  it('should validate an array with optional elements', () => {
    ...
    expect(() => validate(validation, ['wrong', 12])).toThrow((assert) =>
  ...
  it('should validate an object with optional properties', () => {
    ...
    expect(validate(schema1, { foo: 'hello', bar: 50 })).toEqual({ foo: 'hello', bar: 50 })

    expect(validate(schema1, { foo: 'hello' })).toEqual({ foo: 'hello' })

    expect(() => validate(schema1, { foo: 'hello', bar: 40, baz: 60 })).toThrow((assert) =>
      assert.toBeError(ValidationError, /^Found 2 validation errors/).toHaveProperty(
        'errors',
        expect.toMatchContents([
  ...
  it('should validate an object with optional properties and defaults', () => {
    ...
    expect(validate(schema, {})).toEqual({ foo: 'hello', bar: 10, baz: 'world' })

    expect(validate(schema, { foo: 'world', bar: 15, baz: 'hello' })).toEqual({ foo: 'world', bar: 15, baz: 'hello' })

    expect(() => validate(schema, { foo: 'nope', bar: 0, baz: 0 })).toThrow((assert) =>
      assert.toBeError(ValidationError, /^Found 4 validation errors/).toHaveProperty(
        'errors',
        expect.toMatchContents([
  ...
  it('should validate an object when a validator is forced as "optional"', () => {
    ...
    expect(() => validate({ test: validator }, { test: 123 })).toThrow((assert) =>
      assert
        .toBeError(ValidationError, /^Found 1 validation error/)
        .toHaveProperty('errors', expect.toMatchContents([{ path: ['test'], message: 'Value is not a "string"' }])),
    )
  })
  ...
  it('should fail when a default value does not match the validation', () => {
    expect(() =>
      ...
      assert.toBeError(TypeError, 'Default value does not match validator').toHaveProperty(
        'cause',
        expect.toBeError(ValidationError, /^Found 2 validation errors/).toHaveProperty(
          'errors',
          expect.toMatchContents([
  ...
  it('should strip an object with optional properties', () => {
    ...
    expect(strip(schema1, { foo: 'hello', bar: 50, extra: 'foo' })).toEqual({ foo: 'hello', bar: 50 })

    expect(strip(schema1, { foo: 'hello', bar: null, extra: 'foo' })).toEqual({ foo: 'hello' })

    expect(strip(schema1, { foo: 'hello', extra: 'foo' })).toEqual({ foo: 'hello' })

    expect(() => strip(schema1, { foo: 'hello', bar: 40, baz: 60, extra: 'foo' })).toThrow((assert) =>
      assert
        .toBeError(ValidationError, /^Found 1 validation error/)
        .toHaveProperty('errors', expect.toMatchContents([{ path: ['bar'], message: 'Number is less than 50' }])),
    )
  })
})
