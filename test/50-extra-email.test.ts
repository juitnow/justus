...
describe('Extra Email validator', () => {
  it('should validate and normalize an email', () => {
    expect(validate(email, 'Test@Example.ORG')).toStrictlyEqual('test@example.org')
  })

  it('should reject a short email', () => {
    expect(() => validate(email, 'a@b')).toThrow((assert) =>
  ...
  it('should reject a long email', () => {
    ...
    expect(() => validate(email, `${localPart}@example.com`)).toThrow((assert) =>
      assert.toBeError(ValidationError, /^Found 1 validation error$/m).toHaveProperty('errors', (assert) =>
        assert.toEqual([
          {
  ...
  it('should reject an invalid email', () => {
    expect(() => validate(email, 'This is not an email')).toThrow((assert) =>
      assert.toBeError(ValidationError, /^Found 1 validation error$/m).toHaveProperty('errors', (assert) =>
        assert.toEqual([
          {
