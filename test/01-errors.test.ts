...
describe('Errors', () => {
  describe('ValidationError', () => {
    it('should create a validation error', () => {
      ...
      expect(error.errors).toEqual([{ path: [], message: 'This is a test' }])
    })
    ...
    it('should create a validation error with a path', () => {
      ...
      expect(error4.errors).toEqual([{ path: ['foo', 'bar', 123, 456, 'baz', 789], message: 'This is a test' }])
    })
    ...
    it('should wrap another ValidationError', () => {
      ...
      expect(error1.errors).toEqual([{ path: ['foo', 123], message: 'This is a test' }])
    })

    it('should create a validation error ignoring certain stack trace elements', () => {
      function _testCreate_(constructorOrPath?: Function | string[], maybeConstructor?: Function): ValidationError {
  ...
  describe('ValidationError Builder', () => {
    ...
    it('should throw when a simple error have been recorded', () => {
      const builder = new ValidationErrorBuilder().record('Hello, world!').record('Something else', 'myKey')

      expect(() => builder.assert(null)).toThrow((assert) =>
    ...
    it('should properly wrap another ValidationError', () => {
      ...
      const builder = new ValidationErrorBuilder().record(error1).record(error2).record(error1, 123).record(error2, 456)
  ...
  describe('Assertions', () => {
    it('should assert a schema error', () => {
      ...
      expect(() => assertSchema(false, 'This should throw')).toThrowError(TypeError, 'This should throw')
    })

    it('should assert a validation error', () => {
      expect(() => assertValidation(true, 'This should not throw')).not.toThrow()
      expect(() => assertValidation(false, 'This should throw')).toThrow((assert) =>
