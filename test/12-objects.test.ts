...
describe('Object validator', () => {
  it('should validate a generic object', () => {
    ...
    expect(() => validate(object, 123)).toThrow((assert) =>
      assert
        .toBeError(ValidationError, /^Found 1 validation error/)
        .toHaveProperty('errors', expect.toMatchContents([{ path: [], message: 'Value is not an "object"' }])),
    )

    expect(() => validate(object, null)).toThrow((assert) =>
      assert
        .toBeError(ValidationError, /^Found 1 validation error/)
        .toHaveProperty('errors', expect.toMatchContents([{ path: [], message: 'Value is "null"' }])),
    )
  })
  ...
  it('should validate an object', () => {
    ...
    expect(() => validate(validator, 123)).toThrow((assert) =>
      assert
        .toBeError(ValidationError, /^Found 1 validation error/)
        .toHaveProperty('errors', expect.toMatchContents([{ path: [], message: 'Value is not an "object"' }])),
    )

    expect(() => validate(validator, null)).toThrow((assert) =>
      assert
        .toBeError(ValidationError, /^Found 1 validation error/)
        .toHaveProperty('errors', expect.toMatchContents([{ path: [], message: 'Value is "null"' }])),
    )
    ...
    expect(() => validate(validator, { a: 'foo', b: 1, c: true })).toThrow((assert) =>
  ...
  it('should validate a complex object', () => {
    ...
    expect(() => validate(validator, { version: 2, contents: [] })).toThrow((assert) =>
      assert.toBeError(ValidationError, /^Found 3 validation errors/).toHaveProperty(
        'errors',
        expect.toMatchContents([
    ...
    expect(() => validate(validator, { version: 3, contents: [{}, 'foo'] })).toThrow((assert) =>
      assert.toBeError(ValidationError, /^Found 5 validation errors/).toHaveProperty(
        'errors',
        expect.toMatchContents([
          { path: ['version'], message: 'Value does not match constant "1" (number)' },
          { path: ['title'], message: 'Required property missing' },
  ...
  it('should validate an object tree', () => {
    ...
    expect(() => validate(object3, { two: { one: { zero: 0 } } })).toThrow((assert) =>
      assert
        .toBeError(ValidationError, /^Found 1 validation error/)
        .toHaveProperty(
          'errors',
          expect.toMatchContents([{ path: ['two', 'one', 'zero'], message: 'Value is not a "string"' }]),
        ),
    )
  })
  ...
  it('should validate an object with additional properties', () => {
    ...
    expect(validate(validator1, { foo: true, bar: 'whatever' })).toEqual({ foo: true, bar: 'whatever' })
    expect(() => validate(validator1, { foo: false, bar: 'whatever', baz: true })).toThrow((assert) =>
      assert.toBeError(ValidationError, /^Found 2 validation errors/).toHaveProperty(
        'errors',
        expect.toMatchContents([
    ...
    expect(validate(validator2, { foo: true, bar: 'whatever' })).toEqual({ foo: true, bar: 'whatever' })
    expect(() => validate(validator2, { foo: false, bar: 'whatever', baz: true })).toThrow((assert) =>
      assert.toBeError(ValidationError, /^Found 2 validation errors/).toHaveProperty(
        'errors',
        expect.toMatchContents([
          { path: ['foo'], message: 'Value does not match constant "true" (boolean)' },
    ...
    expect(validate(validator3, { foo: true, bar: 'whatever' })).toEqual({ foo: true, bar: 'whatever' })
    expect(() => validate(validator3, { foo: false, bar: 'whatever', baz: true })).toThrow((assert) =>
      assert.toBeError(ValidationError, /^Found 2 validation errors/).toHaveProperty(
        'errors',
        expect.toMatchContents([
          { path: ['foo'], message: 'Value does not match constant "true" (boolean)' },
    ...
    expect(() => validate(validator4, { foo: true, bar: 'whatever', baz: true })).toThrow((assert) =>
      assert.toBeError(ValidationError, /^Found 2 validation error/).toHaveProperty(
        'errors',
        expect.toMatchContents([
          { path: ['baz'], message: 'Forbidden property' },
    ...
    expect(validate(validator5, { foo: true, bar: 'whatever' })).toEqual({ foo: true, bar: 'whatever' })
    expect(validate(validator5, { foo: true, bar: undefined, baz: undefined })).toEqual({ foo: true })
    expect(() => validate(validator5, { foo: true, bar: 123, baz: true })).toThrow((assert) =>
      assert.toBeError(ValidationError, /^Found 2 validation errors/).toHaveProperty(
        'errors',
        expect.toMatchContents([
          { path: ['baz'], message: 'Forbidden property' },
  ...
  it('should validate an object of specific values', () => {
    ...
    expect(validate(validator1, { foo: 'FOO', bar: 'BAR' })).toEqual({ foo: 'FOO', bar: 'BAR' })

    expect(() => validate(validator1, { foo: 'FOO', bar: true })).toThrow((assert) =>
      assert
        .toBeError(ValidationError, /^Found 1 validation error/)
        .toHaveProperty('errors', expect.toMatchContents([{ path: ['bar'], message: 'Value is not a "string"' }])),
    )

    const validator2 = objectOf({ test: boolean })
    expect(validate(validator2, { foo: { test: true }, bar: { test: false } })).toEqual({
      foo: { test: true },
      bar: { test: false },
    })

    expect(() => validate(validator2, { foo: { test: 'hello' }, bar: { test: 123 } })).toThrow((assert) =>
      assert.toBeError(ValidationError, /^Found 2 validation errors/).toHaveProperty(
        'errors',
        expect.toMatchContents([
  ...
  it('should strip additional or forbidden properties when asked to do so', () => {
    ...
    // normal validation
    expect(validate(validator, { foo: true })).toEqual({ foo: true })

    // validate with forbidden/unknown properties are "undefined"
    expect(validate(validator, { foo: true, bar: undefined, baz: undefined })).toEqual({ foo: true })

    // validate with forbidden/unknown properties set
    expect(() => validate(validator, { foo: true, bar: 'whatever', baz: 123 })).toThrow((assert) =>
      assert.toBeError(ValidationError, /^Found 2 validation errors/).toHaveProperty(
        'errors',
        expect.toMatchContents([
          { path: ['baz'], message: 'Forbidden property' },
    ...
    expect(strip(validator, { foo: true, bar: 'whatever' })).toEqual({ foo: true })

    // strip with forbidden properties set
    expect(
      validate(
        validator,
        { foo: true, baz: 'whatever' },
        {
    ...
    expect(
      validate(
        validator,
        { foo: true, bar: 'whatever', baz: 'forbidden' },
        {
          stripAdditionalProperties: true,
          stripForbiddenProperties: true,
        },
    ...
    expect(() =>
      ...
      assert
        .toBeError(ValidationError, /^Found 1 validation error/)
        .toHaveProperty('errors', expect.toMatchContents([{ path: ['baz'], message: 'Forbidden property' }])),
    )
    ...
    expect(() =>
      ...
      assert
        .toBeError(ValidationError, /^Found 1 validation error/)
        .toHaveProperty('errors', expect.toMatchContents([{ path: ['bar'], message: 'Unknown property' }])),
    )
  })
  ...
  it('should perform a partial validation', () => {
    ...
    expect(() => partial(validation, { required: 12345 })).toThrow((assert) =>
      assert
        .toBeError(ValidationError, /^Found 1 validation error/)
        .toHaveProperty('errors', expect.toMatchContents([{ path: ['required'], message: 'Value is not a "string"' }])),
    )

    expect(() => partial(validation, { optional: 12345 })).toThrow((assert) =>
      assert
        .toBeError(ValidationError, /^Found 1 validation error/)
        .toHaveProperty(
          'errors',
          expect.toMatchContents([{ path: ['optional'], message: 'Value is not a "boolean"' }]),
        ),
    )

    expect(() => partial(validation, { defaults: 'foo' })).toThrow((assert) =>
      assert
        .toBeError(ValidationError, /^Found 1 validation error/)
        .toHaveProperty('errors', expect.toMatchContents([{ path: ['defaults'], message: 'Value is not a "number"' }])),
    )
  })
  ...
  it('should perform a partial validation with allowed additional propertiees', () => {
    ...
    expect(() => partial(validation, { addition: 12345 })).toThrow((assert) =>
      assert
        .toBeError(ValidationError, /^Found 1 validation error/)
        .toHaveProperty('errors', expect.toMatchContents([{ path: ['addition'], message: 'Value is not a "string"' }])),
    )
  })
  ...
  it('should extends and allow additional properties', () => {
    ...
    expect(validate(object1, { foo: 'bar', bar: 321, baz: 123 })).toEqual({ foo: 'bar', bar: 321, baz: 123 })
    expect(validate(object2, { foo: 'bar', bar: true, baz: 123 })).toEqual({ foo: 'bar', bar: true, baz: 123 })

    expect(() => validate(object1, { foo: 'bar', bar: true, baz: 123 })).toThrow((assert) =>
      assert
        .toBeError(ValidationError, /^Found 1 validation error/)
        .toHaveProperty('errors', expect.toMatchContents([{ path: ['bar'], message: 'Value is not a "number"' }])),
    )
  })
  ...
  it('should extends and remove additional properties', () => {
    ...
    expect(() => validate(object2, { foo: 'bar', bar: 123 })).toThrow((assert) =>
      assert
        .toBeError(ValidationError, /^Found 1 validation error/)
        .toHaveProperty('errors', expect.toMatchContents([{ path: ['bar'], message: 'Unknown property' }])),
    )
